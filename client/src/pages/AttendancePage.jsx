  ...
  useEffect(() => {
    ...
    const startScanner = async () => {
      try {
        ...
        await html5QrCode.start(
          ...
          async (decodedText) => {
            try {
              ...
              const res = await API.post('/supervisor/attendance', {
        ...
        toast.error('Could not start camera. Please verify permission settings.', {
          id: 'qr-scan-err',
        });
        setScannerOpen(false);
      }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-dark-500 dark:text-dark-400 text-sm mt-1">
          Mark student attendance by date, shift, and bus
        </p>
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[minmax(180px,220px)_minmax(0,1fr)] gap-4">
            ...
            <label className="block">
              ...
              <div className="relative">
                ...
                >
                  {buses.map((bus) => (
                    <option key={bus._id} value={bus._id} className="dark:bg-dark-900">
                      {bus.busName} - {bus.route?.name}
                    </option>
                  ))}
                </select>
              </div>
              ...
              <tbody className="divide-y divide-dark-100 dark:divide-dark-600">
                {students.map((booking) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-sm font-medium text-dark-600 dark:text-dark-300">
                      {booking.student?.studentId}
                    </td>
                    <td className="px-6 py-4 text-sm font-bold text-dark-900 dark:text-white">
                      ...
                      {booking.attendance === 'pending' ? (
                        <div className="flex items-center justify-end gap-2">
                          <button
                            ...
                            title="Mark Present (already deducted)"
                          ...
                          <button
                            ...
                            title="Mark Absent (-2 extra tokens)"
                ...
                {students.length === 0 && (
                  <tr>
                    <td
                      colSpan="7"
                      className="px-6 py-12 text-center text-dark-400 dark:text-dark-500"
                    >
      ...
      <div className="bg-primary-50 dark:bg-primary-950/20 rounded-xl p-4 text-sm text-dark-600 dark:text-dark-400 border border-primary-100/50 dark:border-primary-900/30">
        ...
        <p>
          ✅ Present → <strong>0 extra tokens</strong> (1 reserved during booking) | ❌ Absent →{' '}
          <strong>-2 extra tokens</strong> (total 3 tokens)
        </p>
      </div>
      ...
      <Modal isOpen={scannerOpen} onClose={() => setScannerOpen(false)} title="Scan Ticket QR">
        <div className="space-y-4">
          ...
          <button onClick={() => setScannerOpen(false)} className="btn-secondary w-full">
            Cancel
          </button>
        </div>
      </Modal>
